(function($)
{var usedIds=[];$.fn.slidingMenu=function(options)
{var selector=this.selector;var rtl=false;if($('html').data('textdirection')=="rtl"){rtl=true;}
var settings=$.extend({dataJSON:false,backLabel:'Back'},options);return this.each(function()
{var self=this,menu=$(self),data;if(menu.hasClass('sliding-menu'))
{updateWidth();return;}
var menuWidth=menu.outerWidth();if(settings.dataJSON)
...
{var target=$(href),isBack=$(this).hasClass('back'),marginLeft,marginRight;if(rtl===true){marginRight=parseInt(wrapper.css('margin-right'));}
else{marginLeft=parseInt(wrapper.css('margin-left'));}
var menuWidth=menu.width();if($(this).closest('ul').hasClass('menu-panel-root')){currentPanel=rootPanel;}
if(isBack)
{if(href=='#menu-panel-back')
{target=currentPanel.prev();}
if(rtl===true)
properties={marginRight:marginRight+menuWidth};else
properties={marginLeft:marginLeft+menuWidth};wrapper.stop(true,true).animate(properties,'fast');}
else
...
{handler.addClass('nav-has-children dropdown-item').attr('href','#menu-panel-'+id);handler.append('<i class="ft-arrow-right children-in"></i>')}
panel.attr('id','menu-panel-'+id);if(index==0)
{panel.addClass('menu-panel-root');}
else
{panel.addClass('menu-panel');var li=$('<li></li>'),back=$('<a></a>').addClass('nav-has-parent back primary dropdown-item').attr('href','#menu-panel-back');panel.prepend(back);}
...
while(usedIds.indexOf(id)>=0);usedIds.push(id);return id;}
function updateWidth(){var wrapper=$('.sliding-menu-wrapper'),menuPanels=$('.sliding-menu-wrapper ul');if(menuPanels.length){setTimeout(function(){var menuWidth=$(selector).width();wrapper.width(menuPanels.length*menuWidth);menuPanels.each(function(index,item)
{var panel=$(item);panel.width(menuWidth);});wrapper.css('margin-left','');},300);}}};}(jQuery));