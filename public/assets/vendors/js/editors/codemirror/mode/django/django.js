...
var ch=stream.next();if(ch=="\\"){state.escapeNext=true;}}
return "string";};}
function inVariable(stream,state){if(state.waitDot){state.waitDot=false;if(stream.peek()!="."){return "null";}
...
if(state.waitPipe){state.waitPipe=false;if(stream.peek()!="|"){return "null";}
...
if(state.waitProperty){state.waitProperty=false;if(stream.match(/\b(\w+)\b/)){state.waitDot=true;state.waitPipe=true;return "property";}}
...
if(stream.match(/\b(\w+)\b/)&&!state.foundVariable){state.waitDot=true;state.waitPipe=true;return "variable";}
...
function inTag(stream,state){if(state.waitDot){state.waitDot=false;if(stream.peek()!="."){return "null";}
if(stream.match(/\.\W+/)){return "error";}else if(stream.eat(".")){state.waitProperty=true;return "null";}else{throw Error("Unexpected error while waiting for property.");}}
if(state.waitPipe){state.waitPipe=false;if(stream.peek()!="|"){return "null";}
if(stream.match(/\.\W+/)){return "error";}else if(stream.eat("|")){state.waitFilter=true;return "null";}else{throw Error("Unexpected error while waiting for filter.");}}
if(state.waitProperty){state.waitProperty=false;if(stream.match(/\b(\w+)\b/)){state.waitDot=true;state.waitPipe=true;return "property";}}
...
var keywordMatch=stream.match(keywords);if(keywordMatch){if(keywordMatch[0]=="comment"){state.blockCommentTag=true;}
return "keyword";}
if(stream.match(/\b(\w+)\b/)){state.waitDot=true;state.waitPipe=true;return "variable";}
if(stream.match("%}")){state.waitProperty=null;state.waitFilter=null;state.waitDot=null;state.waitPipe=null;if(state.blockCommentTag){state.blockCommentTag=false;state.tokenize=inBlockComment;}else{state.tokenize=tokenBase;}
return "tag";}
stream.next();return "null";}
